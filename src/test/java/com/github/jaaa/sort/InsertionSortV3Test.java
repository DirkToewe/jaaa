...
import static com.github.jaaa.sort.InsertionSortV3.INSERTION_V3_SORTER;
...
@Group
public class InsertionSortV3Test
{
  @Group
  class SortTest extends TestTemplate
  {
    SortTest() {
      super(new StaticMethodsSorter(InsertionSortV3.class) {
        @Override public boolean isStable    () { return INSERTION_V3_SORTER.isStable    (); }
        @Override public boolean isInplace   () { return INSERTION_V3_SORTER.isInplace   (); }
        @Override public boolean isThreadSafe() { return INSERTION_V3_SORTER.isThreadSafe(); }
      });
    }
  }
  ...
  {
    SorterInplaceTest() { super(INSERTION_V3_SORTER); }
  }
  ...
  private static abstract class TestTemplate extends SorterInplaceTestTemplate
  {
    ...
    void sortsQuicklyOrderedAccessWithRangeByte( @ForAll WithRange<@Size(min=0, max=MAX_SIZE) byte[]> sample )
    {
      ...
      InsertionSortV3.sort(from,until, acc);
