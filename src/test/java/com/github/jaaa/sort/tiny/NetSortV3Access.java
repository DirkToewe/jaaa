...
public interface NetSortV3Access extends CompareSwapAccess
{
  ...
  public default void netSortV3( int from, int until )
  {
    ...
    for( int k=0; k < net.length; )
    {
      ...
      if( c > 0 || c==0 && (mi & ord) > (mi & ord >>> d) ) {
        // swap order (see Hex16::swap)
        ord = ord & ~(mi|mj)
          | ((ord & mi) << d)
          | ((ord & mj) >>>d);
