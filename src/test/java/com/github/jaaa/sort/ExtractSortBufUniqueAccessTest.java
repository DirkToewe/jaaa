...
public class ExtractSortBufUniqueAccessTest
{
  ...
  @Group
  class ExtractSortBufUniqueMinL
  {
    ...
    <T extends Comparable<? super T>> void extractsArr         (         WithInsertIndex<T[]>                             sample ) {
      ...
      var ref = sample.getData().clone();
      var tst =    ref.clone();

      int nUnique = new ExtractSortBufUniqueAccess() {
      ...
      }.extractSortBufUniqueMinL(0,tst.length, len);

      assertThat( copyOfRange(tst,0,nUnique) ).isEqualTo( stream(ref).distinct().sorted().limit(len).toArray() );
    ...
    <T extends Comparable<? super T>> void isStableArr         (         WithInsertIndex<T[]>                             sample,         boolean reversed ) {
      ...
      assertThat(
        ...
        stream(ref).filter(new TreeSet<>(cmp)::add).sorted(cmp).limit(len).toArray(Tuple2[]::new)
      );
    ...
    <T extends Comparable<? super T>> void extractsArrWithRange         (         WithInsertIndex<WithRange<T[]>>                             sample ) {
      ...
      int nUnique = new ExtractSortBufUniqueAccess() {
        @Override public void   swap( int i, int j ) { Swap.swap(tst,i,j); }
        @Override public int compare( int i, int j ) { return tst[i].compareTo(tst[j]); }
      }.extractSortBufUniqueMinL(from,until, len);

      assertThat( copyOfRange(tst,from,from+nUnique) ).isEqualTo( stream(ref,from,until).distinct().sorted().limit(len).toArray() );
    ...
    <T extends Comparable<? super T>> void isStableArrWithRange         (         WithInsertIndex<WithRange<T[]>>                             sample,         boolean reversed ) {
      ...
      assertThat(
        ...
        stream(ref,from,until).filter(new TreeSet<>(cmp)::add).sorted(cmp).limit(len).toArray(Tuple2[]::new)
      );
  ...
  class extractSortBufUniqueMaxR
  {
    @Property                         void extractsArrBoolean  ( @ForAll WithInsertIndex<@Size(max=MAX_SIZE) Boolean  []> sample ) { extractsArr(sample); }
    @Property                         void extractsArrByte     ( @ForAll WithInsertIndex<@Size(max=MAX_SIZE) Byte     []> sample ) { extractsArr(sample); }
    @Property                         void extractsArrShort    ( @ForAll WithInsertIndex<@Size(max=MAX_SIZE) Short    []> sample ) { extractsArr(sample); }
    ...
    <T extends Comparable<? super T>> void extractsArr         (         WithInsertIndex<T[]>                             sample ) {
      ...
      int nUnique = new ExtractSortBufUniqueAccess() {
        @Override public void   swap( int i, int j ) { Swap.swap(tst,i,j); }
        @Override public int compare( int i, int j ) { return tst[i].compareTo(tst[j]); }
      }.extractSortBufUniqueMaxR(0,tst.length, len);

      var           tail = ref.clone();
      Arrays.  sort(tail);
      Revert.revert(tail);
      Revert.revert(tail = (T[]) stream(tail).distinct().limit(len).toArray(Comparable[]::new) );
    ...
    <T extends Comparable<? super T>> void isStableArr         (         WithInsertIndex<T[]>                             sample,         boolean reversed ) {
      ...
      var           tail = ref.clone();
      Arrays.  sort(tail, cmp);
      Revert.revert(tail);
      Revert.revert(tail = (Tuple2<T,Integer>[]) stream(tail).filter(new TreeSet<>(cmp)::add).limit(len).toArray(Tuple2[]::new) );
    ...
    <T extends Comparable<? super T>> void extractsArrWithRange         (         WithInsertIndex<WithRange<T[]>>                             sample ) {
      ...
      int nUnique = new ExtractSortBufUniqueAccess() {
        @Override public void   swap( int i, int j ) { Swap.swap(tst,i,j); }
        @Override public int compare( int i, int j ) { return tst[i].compareTo(tst[j]); }
      }.extractSortBufUniqueMaxR(from,until, len);
