...
@Group
public class MergeSortAccessorTest
{
  private static class Acc<T> implements SortAccessorTestTemplate.SortAccessor<T>, MergeSortAccessor<T>
  {
    ...
    @Override public void sort( T arr, int from, int until ) {
      int bufLen = until-from >>> 1;
      mergeSort(arr,from,until, malloc(bufLen),0,bufLen);
    }
  }
  ...
  @Group class SortAccessorTestLarge  implements TestTemplate { @Override public int maxArraySize() { return 1_000_000; } }
}
