...
public class Byte256Test
{
  ...
  {
    Arbitrary<Action<TestState>> append = Arbitraries.integers().between(0,255).withDistribution( uniform() ).map( i -> new Action<TestState>() {
    ...
    Arbitrary<Action<TestState>> rotate = Arbitraries.integers().withDistribution( uniform() ).map( rot -> new Action<TestState>() {
      @Override public TestState run( TestState state ) {
