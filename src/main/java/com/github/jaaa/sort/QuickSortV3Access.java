...
public interface QuickSortV3Access extends InsertionAdaptiveSortAccess, HeapSelectAccess
{
  default              void quickSortV3_sortRun( int from, int until ) { insertionAdaptiveSort(from,until); }
  default IntBinaryOperator quickSortV3_newPivotChooser() {
...
//      assert compare(c,e) <= 0;
  ...
  default void quickSortV3( int from, int until )
  {
    if( from < 0 || from > until )
      throw new IllegalArgumentException();
    new Int2Consumer()
    {
      private final IntBinaryOperator choosePivot = quickSortV3_newPivotChooser();
      @Override public void accept( int from, int until )
      {
        if( until-from <= 16 )
          quickSortV3_sortRun(from,until);
        else {
          // SELECT RANDOM PIVOT
          // -------------------
