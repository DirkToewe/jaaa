...
// STATIC FIELDS
  ...
  public static class ParallelRecMergeSorter implements Sorter
  {
  // STATIC FIELDS
                                                   ...
                                                   TimSortAccessor<T>
    {
      ...
      @Override default CountedCompleter<?> newMergeTask( int height, CountedCompleter<?> completer, T ab, int a0, int aLen, int b0, int bLen, T c, int c0 )
      {
        ...
        return new ParallelRecMergeTask<>(
  ...
  // CONSTRUCTORS
    public ParallelRecMergeSorter( ForkJoinPool _pool ) {
  ...
  public static final Sorter PARALLEL_REC_MERGE_SORTER = new ParallelRecMergeSorter( ForkJoinPool.commonPool() );

// STATIC CONSTRUCTOR

// STATIC METHODS
  public static <T> void sort( T seq, int from, int until, CompareRandomAccessor<? super T> acc ) { PARALLEL_REC_MERGE_SORTER.sort(seq, from,until, acc); }

  public static <T extends Comparable<? super T>> void sort( T[] seq                                                 ) { PARALLEL_REC_MERGE_SORTER.sort(seq,    0,seq.length     ); }
  public static <T extends Comparable<? super T>> void sort( T[] seq, int from, int until                            ) { PARALLEL_REC_MERGE_SORTER.sort(seq, from,until          ); }
  public static <T>                               void sort( T[] seq,                      Comparator<? super T> cmp ) { PARALLEL_REC_MERGE_SORTER.sort(seq,    0,seq.length, cmp); }
  public static <T>                               void sort( T[] seq, int from, int until, Comparator<? super T> cmp ) { PARALLEL_REC_MERGE_SORTER.sort(seq, from,until,      cmp); }

  public static void sort(      byte[] arr, ComparatorByte   cmp ) { PARALLEL_REC_MERGE_SORTER.sort(arr, 0,arr.length, cmp); }
  public static void sort(     short[] arr, ComparatorShort  cmp ) { PARALLEL_REC_MERGE_SORTER.sort(arr, 0,arr.length, cmp); }
  public static void sort(       int[] arr, ComparatorInt    cmp ) { PARALLEL_REC_MERGE_SORTER.sort(arr, 0,arr.length, cmp); }
  public static void sort(      long[] arr, ComparatorLong   cmp ) { PARALLEL_REC_MERGE_SORTER.sort(arr, 0,arr.length, cmp); }
  public static void sort(      char[] arr, ComparatorChar   cmp ) { PARALLEL_REC_MERGE_SORTER.sort(arr, 0,arr.length, cmp); }
  public static void sort(     float[] arr, ComparatorFloat  cmp ) { PARALLEL_REC_MERGE_SORTER.sort(arr, 0,arr.length, cmp); }
  public static void sort(    double[] arr, ComparatorDouble cmp ) { PARALLEL_REC_MERGE_SORTER.sort(arr, 0,arr.length, cmp); }
  public static void sort(   IntBuffer buf, ComparatorInt    cmp ) { PARALLEL_REC_MERGE_SORTER.sort(buf, buf.position(),buf.limit(), cmp); }

  public static void sort(      byte[] arr ) { PARALLEL_REC_MERGE_SORTER.sort(arr, 0,arr.length); }
  public static void sort(     short[] arr ) { PARALLEL_REC_MERGE_SORTER.sort(arr, 0,arr.length); }
  public static void sort(       int[] arr ) { PARALLEL_REC_MERGE_SORTER.sort(arr, 0,arr.length); }
  public static void sort(      long[] arr ) { PARALLEL_REC_MERGE_SORTER.sort(arr, 0,arr.length); }
  public static void sort(      char[] arr ) { PARALLEL_REC_MERGE_SORTER.sort(arr, 0,arr.length); }
  public static void sort(     float[] arr ) { PARALLEL_REC_MERGE_SORTER.sort(arr, 0,arr.length); }
  public static void sort(    double[] arr ) { PARALLEL_REC_MERGE_SORTER.sort(arr, 0,arr.length); }
  public static void sort(   IntBuffer buf ) { PARALLEL_REC_MERGE_SORTER.sort(buf, buf.position(),buf.limit()); }

  public static void sort(      byte[] arr, int from, int until, ComparatorByte   cmp) { PARALLEL_REC_MERGE_SORTER.sort(arr, from,until, cmp); }
  public static void sort(     short[] arr, int from, int until, ComparatorShort  cmp) { PARALLEL_REC_MERGE_SORTER.sort(arr, from,until, cmp); }
  public static void sort(       int[] arr, int from, int until, ComparatorInt    cmp) { PARALLEL_REC_MERGE_SORTER.sort(arr, from,until, cmp); }
  public static void sort(      long[] arr, int from, int until, ComparatorLong   cmp) { PARALLEL_REC_MERGE_SORTER.sort(arr, from,until, cmp); }
  public static void sort(      char[] arr, int from, int until, ComparatorChar   cmp) { PARALLEL_REC_MERGE_SORTER.sort(arr, from,until, cmp); }
  public static void sort(     float[] arr, int from, int until, ComparatorFloat  cmp) { PARALLEL_REC_MERGE_SORTER.sort(arr, from,until, cmp); }
  public static void sort(    double[] arr, int from, int until, ComparatorDouble cmp) { PARALLEL_REC_MERGE_SORTER.sort(arr, from,until, cmp); }
  public static void sort(   IntBuffer buf, int from, int until, ComparatorInt    cmp) { PARALLEL_REC_MERGE_SORTER.sort(buf, from,until, cmp); }

  public static void sort(      byte[] arr, int from, int until ) { PARALLEL_REC_MERGE_SORTER.sort(arr, from,until); }
  public static void sort(     short[] arr, int from, int until ) { PARALLEL_REC_MERGE_SORTER.sort(arr, from,until); }
  public static void sort(       int[] arr, int from, int until ) { PARALLEL_REC_MERGE_SORTER.sort(arr, from,until); }
  public static void sort(      long[] arr, int from, int until ) { PARALLEL_REC_MERGE_SORTER.sort(arr, from,until); }
  public static void sort(      char[] arr, int from, int until ) { PARALLEL_REC_MERGE_SORTER.sort(arr, from,until); }
  public static void sort(     float[] arr, int from, int until ) { PARALLEL_REC_MERGE_SORTER.sort(arr, from,until); }
  public static void sort(    double[] arr, int from, int until ) { PARALLEL_REC_MERGE_SORTER.sort(arr, from,until); }
  public static void sort(   IntBuffer buf, int from, int until ) { PARALLEL_REC_MERGE_SORTER.sort(buf, from,until); }
