...
public final class HeapSortFast
{
  // STATIC FIELDS
  public static SorterInPlace HEAP_FAST_SORTER = new SorterInPlace()
  {
    @Override public boolean isStable() { return false; }

    @Override public <T> void sort( T seq, int from, int until, CompareRandomAccessor<T> acc ) { HeapSortFast.sort(seq,from,until,acc); }
    @Override public     void sort(        int from, int until, CompareSwapAccess        acc ) { HeapSortFast.sort(    from,until,acc); }
    ...
    @Override public void sort(  short[] seq, int from, int until                       ) { HeapSortFast.sort(seq, from,until          ); }
    @Override public void sort(  short[] seq,                      ComparatorShort  cmp ) { HeapSortFast.sort(seq,    0,seq.length, cmp); }
    @Override public void sort(  short[] seq, int from, int until, ComparatorShort  cmp ) { HeapSortFast.sort(seq, from,until,      cmp); }
    ...
    @Override public void sort(   long[] seq, int from, int until                       ) { HeapSortFast.sort(seq, from,until          ); }
    @Override public void sort(   long[] seq,                      ComparatorLong   cmp ) { HeapSortFast.sort(seq,    0,seq.length, cmp); }
    @Override public void sort(   long[] seq, int from, int until, ComparatorLong   cmp ) { HeapSortFast.sort(seq, from,until,      cmp); }
    ...
    @Override public void sort( double[] seq, int from, int until                       ) { HeapSortFast.sort(seq, from,until          ); }
    @Override public void sort( double[] seq,                      ComparatorDouble cmp ) { HeapSortFast.sort(seq,    0,seq.length, cmp); }
    @Override public void sort( double[] seq, int from, int until, ComparatorDouble cmp ) { HeapSortFast.sort(seq, from,until,      cmp); }

    @Override public void sort( IntBuffer buf                                          ) { HeapSortFast.sort(buf, buf.position(),buf.limit()     ); }
    @Override public void sort( IntBuffer buf , int from, int until                    ) { HeapSortFast.sort(buf,           from,until           ); }
    @Override public void sort( IntBuffer buf ,                      ComparatorInt cmp ) { HeapSortFast.sort(buf, buf.position(),buf.limit(), cmp); }
    @Override public void sort( IntBuffer buf , int from, int until, ComparatorInt cmp ) { HeapSortFast.sort(buf,           from,until,       cmp); }

    @Override public <T extends Comparable<? super T>> void sort( T[] seq                                                 )  { HeapSortFast.sort(seq,    0,seq.length     ); }
    @Override public <T extends Comparable<? super T>> void sort( T[] seq, int from, int until                            )  { HeapSortFast.sort(seq, from,until          ); }
    @Override public <T>                               void sort( T[] seq,                      Comparator<? super T> cmp )  { HeapSortFast.sort(seq,    0,seq.length, cmp); }
    @Override public <T>                               void sort( T[] seq, int from, int until, Comparator<? super T> cmp )  { HeapSortFast.sort(seq, from,until,      cmp); }
  };
...
// STATIC CONSTRUCTOR
  ...
  // FIELDS
// CONSTRUCTORS
  private HeapSortFast() { throw new UnsupportedOperationException("Static class cannot be instantiated."); }
// METHODS
}
