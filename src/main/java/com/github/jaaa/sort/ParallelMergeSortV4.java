...
public class ParallelMergeSortV4
{
// STATIC FIELDS
                                                  ...
                                                  BinaryMergeOffsetAccessor<T[]>
  {
    ...
    @Override public void      sort( T[] arr, int arr0, T[] buf, int buf0, int len ) {
      ComparatorTimSort.sort(arr, arr0,arr0+len, cmp, buf,buf0,len);
    }
    @Override public int mergeOffset(T[] ab, int a0, int aLen, int b0, int bLen, int nSkip ) {
      return binaryMergeOffset(ab,a0,aLen, ab,b0,bLen, nSkip);
    }
  }
...
// CONSTRUCTORS
  private ParallelMergeSortV4() { throw new UnsupportedOperationException("Static class cannot be instantiated."); }
