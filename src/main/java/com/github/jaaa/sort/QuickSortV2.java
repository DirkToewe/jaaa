...
public final class QuickSortV2
{
  // STATIC FIELDS
  public static SorterInPlace QUICK_V2_SORTER = new SorterInPlace()
  {
    @Override public boolean isStable() { return false; }

    @Override public <T> void sort( T seq, int from, int until, CompareRandomAccessor<T> acc ) { QuickSortV2.sort(seq,from,until,acc); }
    @Override public     void sort(        int from, int until, CompareSwapAccess        acc ) { QuickSortV2.sort(    from,until,acc); }

    @Override public void sort(   byte[] seq                                          ) { QuickSortV2.sort(seq,    0,seq.length     ); }
    @Override public void sort(   byte[] seq, int from, int until                     ) { QuickSortV2.sort(seq, from,until          ); }
    @Override public void sort(   byte[] seq,                      ComparatorByte cmp ) { QuickSortV2.sort(seq,    0,seq.length, cmp); }
    @Override public void sort(   byte[] seq, int from, int until, ComparatorByte cmp ) { QuickSortV2.sort(seq, from,until,      cmp); }

    @Override public void sort(  short[] seq                                           ) { QuickSortV2.sort(seq,    0,seq.length     ); }
    @Override public void sort(  short[] seq, int from, int until                      ) { QuickSortV2.sort(seq, from,until          ); }
    @Override public void sort(  short[] seq,                      ComparatorShort cmp ) { QuickSortV2.sort(seq,    0,seq.length, cmp); }
    @Override public void sort(  short[] seq, int from, int until, ComparatorShort cmp ) { QuickSortV2.sort(seq, from,until,      cmp); }

    @Override public void sort(    int[] seq                                         ) { QuickSortV2.sort(seq,    0,seq.length     ); }
    @Override public void sort(    int[] seq, int from, int until                    ) { QuickSortV2.sort(seq, from,until          ); }
    @Override public void sort(    int[] seq,                      ComparatorInt cmp ) { QuickSortV2.sort(seq,    0,seq.length, cmp); }
    @Override public void sort(    int[] seq, int from, int until, ComparatorInt cmp ) { QuickSortV2.sort(seq, from,until,      cmp); }

    @Override public void sort(   long[] seq                                          ) { QuickSortV2.sort(seq,    0,seq.length     ); }
    @Override public void sort(   long[] seq, int from, int until                     ) { QuickSortV2.sort(seq, from,until          ); }
    @Override public void sort(   long[] seq,                      ComparatorLong cmp ) { QuickSortV2.sort(seq,    0,seq.length, cmp); }
    @Override public void sort(   long[] seq, int from, int until, ComparatorLong cmp ) { QuickSortV2.sort(seq, from,until,      cmp); }

    @Override public void sort(   char[] seq                                          ) { QuickSortV2.sort(seq,    0,seq.length     ); }
    @Override public void sort(   char[] seq, int from, int until                     ) { QuickSortV2.sort(seq, from,until          ); }
    @Override public void sort(   char[] seq,                      ComparatorChar cmp ) { QuickSortV2.sort(seq,    0,seq.length, cmp); }
    @Override public void sort(   char[] seq, int from, int until, ComparatorChar cmp ) { QuickSortV2.sort(seq, from,until,      cmp); }

    @Override public void sort(  float[] seq                                           ) { QuickSortV2.sort(seq,    0,seq.length     ); }
    @Override public void sort(  float[] seq, int from, int until                      ) { QuickSortV2.sort(seq, from,until          ); }
    @Override public void sort(  float[] seq,                      ComparatorFloat cmp ) { QuickSortV2.sort(seq,    0,seq.length, cmp); }
    @Override public void sort(  float[] seq, int from, int until, ComparatorFloat cmp ) { QuickSortV2.sort(seq, from,until,      cmp); }

    @Override public void sort( double[] seq                                            ) { QuickSortV2.sort(seq,    0,seq.length     ); }
    @Override public void sort( double[] seq, int from, int until                       ) { QuickSortV2.sort(seq, from,until          ); }
    @Override public void sort( double[] seq,                      ComparatorDouble cmp ) { QuickSortV2.sort(seq,    0,seq.length, cmp); }
    @Override public void sort( double[] seq, int from, int until, ComparatorDouble cmp ) { QuickSortV2.sort(seq, from,until,      cmp); }

    @Override public void sort( IntBuffer buf                                          ) { QuickSortV2.sort(buf, buf.position(),buf.limit()     ); }
    @Override public void sort( IntBuffer buf , int from, int until                    ) { QuickSortV2.sort(buf,           from,until           ); }
    @Override public void sort( IntBuffer buf ,                      ComparatorInt cmp ) { QuickSortV2.sort(buf, buf.position(),buf.limit(), cmp); }
    @Override public void sort( IntBuffer buf , int from, int until, ComparatorInt cmp ) { QuickSortV2.sort(buf,           from,until,       cmp); }

    @Override public <T extends Comparable<? super T>> void sort( T[] seq                                                 )  { QuickSortV2.sort(seq,    0,seq.length     ); }
    @Override public <T extends Comparable<? super T>> void sort( T[] seq, int from, int until                            )  { QuickSortV2.sort(seq, from,until          ); }
    @Override public <T>                               void sort( T[] seq,                      Comparator<? super T> cmp )  { QuickSortV2.sort(seq,    0,seq.length, cmp); }
    @Override public <T>                               void sort( T[] seq, int from, int until, Comparator<? super T> cmp )  { QuickSortV2.sort(seq, from,until,      cmp); }
  };
...
// STATIC CONSTRUCTOR
  ...
  public static <T> void sort( T[] seq, int from, int until, Comparator<? super T> cmp )
  {
    new QuickSortV2Access() {
      @Override public int compare( int i, int j ) { return cmp.compare( seq[i], seq[j] ); }
      @Override public void   swap( int i, int j ) { Swap.swap(seq,i,j); }
      @Override public void quickSortV2_sortRun( int from, int until ) {
  ...
  // FIELDS
// CONSTRUCTORS
  private QuickSortV2() { throw new UnsupportedOperationException("Static class cannot be instantiated."); }
// METHODS
}
