...
public final class HexInSortV2
{
  ...
  public static SorterInPlace HEX_IN_V2_SORTER = new SorterInPlace()
  {
    ...
    @Override public <T> void sort( T seq, int from, int until, CompareRandomAccessor<T> acc ) { HexInSortV2.sort(seq,from,until,acc); }
    ...
    @Override public void sort(     short[] seq, int from, int until                       ) { HexInSortV2.sort(seq, from,until          ); }
    ...
    @Override public void sort(      long[] seq, int from, int until                       ) { HexInSortV2.sort(seq, from,until          ); }
    ...
    @Override public void sort(    double[] seq, int from, int until                       ) { HexInSortV2.sort(seq, from,until          ); }
    ...
    @Override public void sort(   IntBuffer buf                                            ) { HexInSortV2.sort(buf, buf.position(),buf.limit()     ); }
    @Override public void sort(   IntBuffer buf, int from, int until                       ) { HexInSortV2.sort(buf,           from,until           ); }
    @Override public void sort(   IntBuffer buf,                      ComparatorInt    cmp ) { HexInSortV2.sort(buf, buf.position(),buf.limit(), cmp); }
    @Override public void sort(   IntBuffer buf, int from, int until, ComparatorInt    cmp ) { HexInSortV2.sort(buf,           from,until,       cmp); }

    @Override public <T extends Comparable<? super T>> void sort( T[] seq                                                 )  { HexInSortV2.sort(seq,    0,seq.length     ); }
    @Override public <T extends Comparable<? super T>> void sort( T[] seq, int from, int until                            )  { HexInSortV2.sort(seq, from,until          ); }
