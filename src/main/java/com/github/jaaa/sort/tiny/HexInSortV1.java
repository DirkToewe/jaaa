package com.github.jaaa.sort.tiny;

import com.github.jaaa.CompareSwapAccess;
import com.github.jaaa.permute.Swap;
import com.github.jaaa.compare.*;
import com.github.jaaa.sort.SorterInPlace;

import java.nio.IntBuffer;
import java.util.Comparator;

public final class HexInSortV1
{
// STATIC FIELDS
  public static SorterInPlace HEX_IN_V1_SORTER = new SorterInPlace()
  {
    @Override public boolean isStable() { return true; }

    @Override public <T> void sort( T seq, int from, int until, CompareRandomAccessor<T> acc ) { HexInSortV1.sort(seq,from,until,acc); }
    @Override public     void sort(        int from, int until, CompareSwapAccess        acc ) { HexInSortV1.sort(    from,until,acc); }

    @Override public void sort(      byte[] seq                                            ) { HexInSortV1.sort(seq,    0,seq.length     ); }
    @Override public void sort(      byte[] seq, int from, int until                       ) { HexInSortV1.sort(seq, from,until          ); }
    @Override public void sort(      byte[] seq,                      ComparatorByte cmp ) { HexInSortV1.sort(seq,    0,seq.length, cmp); }
    @Override public void sort(      byte[] seq, int from, int until, ComparatorByte   cmp ) { HexInSortV1.sort(seq, from,until,      cmp); }

    @Override public void sort(     short[] seq                                            ) { HexInSortV1.sort(seq,    0,seq.length     ); }
    @Override public void sort(     short[] seq, int from, int until                       ) { HexInSortV1.sort(seq, from,until          ); }
    @Override public void sort(     short[] seq,                      ComparatorShort  cmp ) { HexInSortV1.sort(seq,    0,seq.length, cmp); }
    @Override public void sort(     short[] seq, int from, int until, ComparatorShort  cmp ) { HexInSortV1.sort(seq, from,until,      cmp); }

    @Override public void sort(       int[] seq                                            ) { HexInSortV1.sort(seq,    0,seq.length     ); }
    @Override public void sort(       int[] seq, int from, int until                       ) { HexInSortV1.sort(seq, from,until          ); }
    @Override public void sort(       int[] seq,                      ComparatorInt cmp ) { HexInSortV1.sort(seq,    0,seq.length, cmp); }
    @Override public void sort(       int[] seq, int from, int until, ComparatorInt    cmp ) { HexInSortV1.sort(seq, from,until,      cmp); }

    @Override public void sort(      long[] seq                                            ) { HexInSortV1.sort(seq,    0,seq.length     ); }
    @Override public void sort(      long[] seq, int from, int until                       ) { HexInSortV1.sort(seq, from,until          ); }
    @Override public void sort(      long[] seq,                      ComparatorLong   cmp ) { HexInSortV1.sort(seq,    0,seq.length, cmp); }
    @Override public void sort(      long[] seq, int from, int until, ComparatorLong   cmp ) { HexInSortV1.sort(seq, from,until,      cmp); }

    @Override public void sort(      char[] seq                                            ) { HexInSortV1.sort(seq,    0,seq.length     ); }
    @Override public void sort(      char[] seq, int from, int until                       ) { HexInSortV1.sort(seq, from,until          ); }
    @Override public void sort(      char[] seq,                      ComparatorChar cmp ) { HexInSortV1.sort(seq,    0,seq.length, cmp); }
    @Override public void sort(      char[] seq, int from, int until, ComparatorChar   cmp ) { HexInSortV1.sort(seq, from,until,      cmp); }

    @Override public void sort(     float[] seq                                            ) { HexInSortV1.sort(seq,    0,seq.length     ); }
    @Override public void sort(     float[] seq, int from, int until                       ) { HexInSortV1.sort(seq, from,until          ); }
    @Override public void sort(     float[] seq,                      ComparatorFloat cmp ) { HexInSortV1.sort(seq,    0,seq.length, cmp); }
    @Override public void sort(     float[] seq, int from, int until, ComparatorFloat  cmp ) { HexInSortV1.sort(seq, from,until,      cmp); }

    @Override public void sort(    double[] seq                                            ) { HexInSortV1.sort(seq,    0,seq.length     ); }
    @Override public void sort(    double[] seq, int from, int until                       ) { HexInSortV1.sort(seq, from,until          ); }
    @Override public void sort(    double[] seq,                      ComparatorDouble cmp ) { HexInSortV1.sort(seq,    0,seq.length, cmp); }
    @Override public void sort(    double[] seq, int from, int until, ComparatorDouble cmp ) { HexInSortV1.sort(seq, from,until,      cmp); }

    @Override public void sort(   IntBuffer buf                                         ) { HexInSortV1.sort(buf, buf.position(),buf.limit()     ); }
    @Override public void sort(   IntBuffer buf, int from, int until                    ) { HexInSortV1.sort(buf,           from,until           ); }
    @Override public void sort(   IntBuffer buf,                      ComparatorInt cmp ) { HexInSortV1.sort(buf, buf.position(),buf.limit(), cmp); }
    @Override public void sort(   IntBuffer buf, int from, int until, ComparatorInt cmp ) { HexInSortV1.sort(buf,           from,until,       cmp); }

    @Override public <T extends Comparable<? super T>> void sort( T[] seq                                                 )  { HexInSortV1.sort(seq,    0,seq.length     ); }
    @Override public <T extends Comparable<? super T>> void sort( T[] seq, int from, int until                            )  { HexInSortV1.sort(seq, from,until          ); }
    @Override public <T>                               void sort( T[] seq,                      Comparator<? super T> cmp )  { HexInSortV1.sort(seq,    0,seq.length, cmp); }
    @Override public <T>                               void sort( T[] seq, int from, int until, Comparator<? super T> cmp )  { HexInSortV1.sort(seq, from,until,      cmp); }
  };

// STATIC CONSTRUCTOR

// STATIC METHODS
  public static <T> void sort( T seq, int from, int until, CompareRandomAccessor<? super T> acc )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return acc.compare(seq,i, seq,j); }
      @Override public void   swap( int i, int j ) {        acc.   swap(seq,i, seq,j); }
    }.hexInSortV1(from,until);
  }

  public static void sort( int from, int until, CompareSwapAccess acc )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return acc.compare(i,j); }
      @Override public void   swap( int i, int j ) {        acc.   swap(i,j); }
    }.hexInSortV1(from,until);
  }

  public static void sort( byte[] seq, int from, int until )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return Byte.compare( seq[i], seq[j] ); }
      @Override public void   swap( int i, int j ) { Swap.swap(seq,i,j); }
    }.hexInSortV1(from,until);
  }

  public static void sort( byte[] seq, int from, int until, ComparatorByte cmp )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return cmp.compare( seq[i], seq[j] ); }
      @Override public void   swap( int i, int j ) { Swap.swap(seq,i,j); }
    }.hexInSortV1(from,until);
  }

  public static void sort( short[] seq, int from, int until )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return Short.compare( seq[i], seq[j] ); }
      @Override public void   swap( int i, int j ) { Swap.swap(seq,i,j); }
    }.hexInSortV1(from,until);
  }

  public static void sort( short[] seq, int from, int until, ComparatorShort cmp )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return cmp.compare( seq[i], seq[j] ); }
      @Override public void   swap( int i, int j ) { Swap.swap(seq,i,j); }
    }.hexInSortV1(from,until);
  }

  public static void sort( int[] seq, int from, int until )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return Integer.compare( seq[i], seq[j] ); }
      @Override public void   swap( int i, int j ) { Swap.swap(seq,i,j); }
    }.hexInSortV1(from,until);
  }

  public static void sort( int[] seq, int from, int until, ComparatorInt cmp )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return cmp.compare( seq[i], seq[j] ); }
      @Override public void   swap( int i, int j ) { Swap.swap(seq,i,j); }
    }.hexInSortV1(from,until);
  }

  public static void sort( long[] seq, int from, int until )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return Long.compare( seq[i], seq[j] ); }
      @Override public void   swap( int i, int j ) { Swap.swap(seq,i,j); }
    }.hexInSortV1(from,until);
  }

  public static void sort( long[] seq, int from, int until, ComparatorLong cmp )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return cmp.compare( seq[i], seq[j] ); }
      @Override public void   swap( int i, int j ) { Swap.swap(seq,i,j); }
    }.hexInSortV1(from,until);
  }

  public static void sort( char[] seq, int from, int until )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return Character.compare( seq[i], seq[j] ); }
      @Override public void   swap( int i, int j ) { Swap.swap(seq,i,j); }
    }.hexInSortV1(from,until);
  }

  public static void sort( char[] seq, int from, int until, ComparatorChar cmp )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return cmp.compare( seq[i], seq[j] ); }
      @Override public void   swap( int i, int j ) { Swap.swap(seq,i,j); }
    }.hexInSortV1(from,until);
  }

  public static void sort( float[] seq, int from, int until )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return Float.compare( seq[i], seq[j] ); }
      @Override public void   swap( int i, int j ) { Swap.swap(seq,i,j); }
    }.hexInSortV1(from,until);
  }

  public static void sort( float[] seq, int from, int until, ComparatorFloat cmp )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return cmp.compare( seq[i], seq[j] ); }
      @Override public void   swap( int i, int j ) { Swap.swap(seq,i,j); }
    }.hexInSortV1(from,until);
  }

  public static void sort( double[] seq, int from, int until )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return Double.compare( seq[i], seq[j] ); }
      @Override public void   swap( int i, int j ) { Swap.swap(seq,i,j); }
    }.hexInSortV1(from,until);
  }

  public static void sort( double[] seq, int from, int until, ComparatorDouble cmp )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return cmp.compare( seq[i], seq[j] ); }
      @Override public void   swap( int i, int j ) { Swap.swap(seq,i,j); }
    }.hexInSortV1(from,until);
  }

  public static void sort( IntBuffer buf, int from, int until )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return Integer.compare( buf.get(i), buf.get(j) ); }
      @Override public void   swap( int i, int j ) { Swap.swap(buf,i,j); }
    }.hexInSortV1(from,until);
  }

  public static void sort( IntBuffer buf, int from, int until, ComparatorInt cmp )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return cmp.compare( buf.get(i), buf.get(j) ); }
      @Override public void   swap( int i, int j ) { Swap.swap(buf,i,j); }
    }.hexInSortV1(from,until);
  }

  public static <T extends Comparable<? super T>> void sort( T[] seq, int from, int until )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return seq[i].compareTo(seq[j]); }
      @Override public void   swap( int i, int j ) { Swap.swap(seq,i,j); }
    }.hexInSortV1(from,until);
  }

  public static <T> void sort( T[] seq, int from, int until, Comparator<? super T> cmp )
  {
    new HexInSortV1Access() {
      @Override public int compare( int i, int j ) { return cmp.compare( seq[i], seq[j] ); }
      @Override public void   swap( int i, int j ) { Swap.swap(seq,i,j); }
    }.hexInSortV1(from,until);
  }

  public static void sort(      byte[] seq                       ) { sort(seq, 0,seq.length     ); }
  public static void sort(      byte[] seq, ComparatorByte   cmp ) { sort(seq, 0,seq.length, cmp); }

  public static void sort(     short[] seq                       ) { sort(seq, 0,seq.length     ); }
  public static void sort(     short[] seq, ComparatorShort  cmp ) { sort(seq, 0,seq.length, cmp); }

  public static void sort(       int[] seq                       ) { sort(seq, 0,seq.length     ); }
  public static void sort(       int[] seq, ComparatorInt    cmp ) { sort(seq, 0,seq.length, cmp); }

  public static void sort(      long[] seq                       ) { sort(seq, 0,seq.length     ); }
  public static void sort(      long[] seq, ComparatorLong   cmp ) { sort(seq, 0,seq.length, cmp); }

  public static void sort(      char[] seq                       ) { sort(seq, 0,seq.length     ); }
  public static void sort(      char[] seq, ComparatorChar   cmp ) { sort(seq, 0,seq.length, cmp); }

  public static void sort(     float[] seq                       ) { sort(seq, 0,seq.length     ); }
  public static void sort(     float[] seq, ComparatorFloat  cmp ) { sort(seq, 0,seq.length, cmp); }

  public static void sort(    double[] seq                       ) { sort(seq, 0,seq.length     ); }
  public static void sort(    double[] seq, ComparatorDouble cmp ) { sort(seq, 0,seq.length, cmp); }

  public static void sort(   IntBuffer buf                       ) { sort(buf, buf.position(),buf.limit()     ); }
  public static void sort(   IntBuffer buf, ComparatorInt    cmp ) { sort(buf, buf.position(),buf.limit(), cmp); }

  public static <T extends Comparable<? super T>> void sort( T[] seq                            )  { sort(seq, 0,seq.length     ); }
  public static <T>                               void sort( T[] seq, Comparator<? super T> cmp )  { sort(seq, 0,seq.length, cmp); }

// FIELDS
// CONSTRUCTORS
  private HexInSortV1() { throw new UnsupportedOperationException("Static class cannot be instantiated."); }
// METHODS
}
