...
public interface MinHeapLAccess extends CompareSwapAccess
{
  default void minHeapL_build( int from, int until )
  {
    ...
    for( int root=firstChild; from < root--; )
      for( int parent=root;; )
      {
        ...
        if( child < until )
            child += compare(1+child,child)>>>31;
  ...
  default void minHeapL_buildFast( int from, int until )
  {
    // REFERENCES
    // ----------
  ...
  default int minHeapL_child( int from, int until, int parent )
  {
  ...
  default int minHeapL_parent( int from, int until, int child )
  {
  ...
  default void minHeapL_siftDown( int from, int until, int parent )
  {
    if( from < 0 || from > parent || parent >= until )
      throw new IllegalArgumentException();
    --until;
  ...
  default void minHeapL_siftDownFast( int from, int until, int parent )
  {
    if( from < 0 || from > parent || parent >= until )
      throw new IllegalArgumentException();
    --until;
  ...
  default void minHeapL_bubbleUp( int from, int until, int child )
  {
    if( from < 0 || from > child || child >= until )
      throw new IllegalArgumentException();
    // BUBBLE UP
