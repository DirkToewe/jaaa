...
public interface MinHeapRAccess extends CompareSwapAccess
{
  default void minHeapR_build( int from, int until )
  {
  ...
  default void minHeapR_buildFast( int from, int until )
  {
    // REFERENCES
    // ----------
  ...
  default int minHeapR_child( int from, int until, int parent )
  {
  ...
  default int minHeapR_parent( int from, int until, int child )
  {
  ...
  default void minHeapR_siftDown( int from, int until, int parent )
  {
    if( from < 0 || from > parent || parent >= until )
      throw new IllegalArgumentException();
    for(;;) {
  ...
  default void minHeapR_siftDownFast( int from, int until, int parent )
  {
    if( from < 0 || from > parent || parent >= until )
      throw new IllegalArgumentException();
    int p = parent;
  ...
  default void minHeapR_bubbleUp( int from, int until, int child )
  {
    if( from < 0 || from > child || child >= until )
      throw new IllegalArgumentException();
    // BUBBLE UP
