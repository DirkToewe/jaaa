...
public interface MergeSelectStableAccessor<T> extends ArgMaxAccessor<T>,
                                                      ArgMinAccessor<T>,
                                              ExpMergeOffsetAccessor<T>,
                                               InsertionSortAccessor<T>,
                                                   TapeMergeAccessor<T>
{
  default void mergeSelectStable_sortRun( T a, int from, int until ) { insertionSort(a,from,until); }
  default  int mergeSelectStable_runLen( int len, int nSelect ) {
  ...
  default int mergeSelectStable_mergeOffset( T ab, int a0, int aLen, int b0, int bLen, int nSkip ) {
    return expMergeOffset(ab,a0,aLen, ab,b0,bLen, nSkip);
  }

  default void mergeSelectStable_mergeL2R(
    T a, int a0, int aLen,
    T b, int b0, int bLen,
    T c, int c0
  ) {
  ...
  default void mergeSelectStable_mergeR2L(
    T a, int a0, int aLen,
    T b, int b0, int bLen,
    T c, int c0
  ) {
  ...
  default void mergeSelectStable( T arr, int from, int mid, int until, T buf, int buf0, int buf1 ) {
    if( mid-from <= until-mid )
      mergeSelectStableL(arr, from,mid,until, buf,buf0,buf1);
    else
      mergeSelectStableR(arr, from,mid,until, buf,buf0,buf1);
  }

  default void mergeSelectStableL( T arr, int from, int mid, int until, T buff, int buff0, int buff1 )
  {
    ...
    new Object() {
      ...
      void merge( int j, int lenL, int lenR ) {
        ...
        nSelect = min(lenL+lenR, N_SELECT),
             nL = mergeSelectStable_mergeOffset(arr, i,lenL, j,lenR, nSelect),
             nR = nSelect - nL;
        ...
        if( 0 < nR ) {
          ...
          copyRange(arr,i+nL, arr,i+nSelect, lenL-nL);
          mergeSelectStable_mergeR2L(arr,i,nL, buf,buf0,nR, arr,i);
        }
      }

      {
        final int RUN_LEN = mergeSelectStable_runLen(until-from, N_SELECT);
        ...
        for( int i=from; i < until; ) {
          ...
          mergeSelectStable_sortRun(arr, i,nxt);
  ...
  default void mergeSelectStableR( T arr, int from, int mid, int until, T buff, int buff0, int buff1 )
  {
    if( from < 0 || from > mid || mid > until || buff0 < 0 || buff0 > buff1 )
      throw new IndexOutOfBoundsException();
    ...
    new Object() {
      ...
      void merge( int j, int lenL, int lenR ) {
        ...
        nSelect = min(len, N_SELECT),
          skip  = len - nSelect,
          skipL = mergeSelectStable_mergeOffset(arr, i,lenL, j,lenR, skip),
          skipR = skip - skipL,
             nL = lenL - skipL,
             nR = lenR - skipR;
        assert nL >= 0;
        ...
        if( 0 < nL ) {
          ...
          copyRange(arr,i+ lenL, arr,i+skipL, skipR);
          mergeSelectStable_mergeL2R(buf,buf0,nL, arr,j+skipR,nR, arr,i+skip);
        }
      }

      {
        ...
        for( int i=until; i > from; ) {
          ...
          mergeSelectStable_sortRun(arr,nxt,i);
