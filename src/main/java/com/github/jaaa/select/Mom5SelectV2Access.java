...
public interface Mom5SelectV2Access extends HeapSelectAccess, BlockSwapAccess
{
  default void mom5SelectV2_median5( int a, int c, int d )
  {
    int b = a+1,
        e = d+1;
  ...
  default long mom5SelectV2_smallSelect_performance( int from, int mid, int until ) {
    return heapSelectMajor_performance(from,mid,until);
  }
  default void mom5SelectV2_smallSelect( int from, int mid, int until ) {
    heapSelectMajor(from,mid,until);
  }

  default void mom5SelectV2( int from, final int mid, int until )
  {
    if( from < 0 || from > mid || mid > until )
      throw new IllegalArgumentException();

    while( from < mid && mid < until-1 && until-from > 2 && mom5SelectV2_smallSelect_performance(from,mid,until) > 4.6*(until-from) )
    {
      int       len = until - from;
      int fth = len/5,
          rem = len-5*fth;
      ...
      for( int lhs = from + fth*2, m = lhs; i < lhs; ) {
        j -= 2; mom5SelectV2_median5(i,m,j);
        i += 2;
        m += 1;
      }
      ...
      if( rem != 0 ) {
        int rhs = j,
            sel =(j -= rem) + rem/2;
        mom5SelectV2_smallSelect(j, sel, rhs);
        swap(sel,j++);
        fth++;
      }
    ...
    mom5SelectV2_smallSelect(from,mid,until);
  }
}
