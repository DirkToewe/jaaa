...
public interface HeapSelectHybridAccess extends CompareSwapAccess, ArgMaxAccess, ArgMinAccess
{
  default void heapSelectHybridV1( int from, int mid, int until )
  {
    if( mid-from >= until-mid ) heapSelectHybridL(from,mid,until);
    else                        heapSelectHybridR(from,mid,until);
  }

  default void heapSelectHybridV2( int from, int mid, int until )
  {
    if( mid-from <= until-mid ) heapSelectHybridL(from,mid,until);
    else                        heapSelectHybridR(from,mid,until);
  }

  default void heapSelectHybridL( final int from, final int mid, final int until )
  {
  ...
  default void heapSelectHybridR( int from, int mid, int until )
  {
    if( from < 0 || from > mid || mid > until )
      throw new IllegalArgumentException();
