 ...
 */
public interface Mom3SelectAccess extends HeapSelectAccess, BlockSwapAccess
{
  default void mom3Select_median3(int a, int b, int c )
  {
  ...
  default long mom3Select_smallSelect_performance( int from, int mid, int until ) {
    return heapSelect_performance(from,mid,until);
  }
  default void mom3Select_smallSelect( int from, int mid, int until ) {
    heapSelect(from,mid,until);
  }

  default void mom3Select( int from, final int mid, int until )
  {
    if( from < 0 || from > mid || mid > until )
      throw new IllegalArgumentException();

    while( from < mid && mid < until-1 && until-from > 8 && mom3Select_smallSelect_performance(from,mid,until) > 4.8*(until-from) )
    {
      int i = from,
          j = until,
        trd = until - from;
      ...
      for( int repeat=2; repeat-- > 0; )
      {
        ...
        int rhs = j,
            sel =(j -= rem) + rem/2;
        mom3Select_smallSelect(j, sel, rhs);
        swap(sel,j++);
        trd++;
      }
    ...
    mom3Select_smallSelect(from,mid,until);
  }
}
