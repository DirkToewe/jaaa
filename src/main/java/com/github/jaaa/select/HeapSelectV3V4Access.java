...
public interface HeapSelectV3V4Access extends CompareSwapAccess, ArgMaxAccess, ArgMinAccess
{
  default void heapSelectV3( int from, int mid, int until )
  {
  ...
  default void heapSelectV4( int from, int mid, int until )
  {
    if( from < 0 || from > mid || mid > until ) throw new IllegalArgumentException();
    if( from == mid || mid == until ) return;
  ...
  /** Returns a rough estimate of the worst case performance of {@link #heapSelectV3(int,int,int)}.
   */
  default long heapSelectV3_worstCasePerformance( int from, int mid, int until )
  {
    if( from < 0 || from > mid || mid > until ) throw new IllegalArgumentException();
    long s,l; {
      int m =   mid-from,
