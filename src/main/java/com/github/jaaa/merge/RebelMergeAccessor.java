...
public interface RebelMergeAccessor<T> extends CompareRandomAccessor<T>, BinarySearchAccessor<T>
{
  ...
  default void rebelMerge(
    ...
    if(  a==c && k < i+m && i <= k
      || b==c && k < j+n && j <= k )
      rebelMergeR2L(a,i,m, b,j,n, c,k); // <- depending on overlap, merge from right to left
    else
      rebelMergeL2R(a,i,m, b,j,n, c,k);
  }

  default void rebelMergeL2R(
    T a, int i, int m,
    T b, int j, int n,
    T c, int k
  ) {
